...
<<?php if ($this->options->InstantClick == '2'||$this->options->InstantClick == '3'): ?>form<?php else: ?>
div<?php endif; ?> class="js-search search-form search-form--modal" method="get" role="search">
		...
		<div class="search_close" onclick="$.Close()"></div></<?php if ($this->options->InstantClick == '2'||$this->options->InstantClick == '3'): ?>form<?php else: ?>
div<?php endif; ?>>

<?php $face=array("主页","分类","归档","标签","友链","关于"); ?><sidebar id="sidebar">
<a  href="<?php $this->options->rootUrl(); ?>/"><div class="tx">
<img class="b-lazy" src="<?php echo theurl; ?>images/load.gif" <?php if ($this->options->logoUrl){ ?>data-src="<?php $this->options->logoUrl();?>"<?php }else{ ?>data-src="//cdn.v2ex.com/gravatar/<?php echo md5(strtolower($this->author->mail)); ?>?s=140"<?php };?> alt="<?php $this->options->title(); ?>" itemprop="image" data-no-instant>
...
<?php endif; ?><?php if (!empty($this->options->sidebarBlock) && in_array('tags', $this->options->sidebarBlock)): ?>
...
<?php if($this->options->skin &&  'old'==$this->options->skin): ?><div class="oldbg"></div><?php endif; ?>
</sidebar>
