...
<div class="meta">
 ...
 <?php if (!empty($this->options->sidebarBlock) && in_array('readnum', $this->options->sidebarBlock)): ?><span>read (<?php get_post_view($this) ?>)</span><?php endif;?> 
   <?php if (!empty($this->options->sidebarBlock) && in_array('zishu', $this->options->sidebarBlock)): ?><span><?php echo art_count($this->cid); ?>汉字</span><?php endif;?> 
...
<div class="content">
  <?php
$loadtu = theurl.'images/load.gif';

$str = preg_replace('#<a(.*?) href="([^"]*/)?(([^"/]*)\.[^"]*)"(.*?)>#',
        '<a$1 href="$2$3"$5 target="_blank">', $this->content);

if(strpos(Typecho_Widget::widget('Widget_Options')->Enhance,'wtyl') !== false){
$str = preg_replace('#<li><img src="([^"]*/)?(([^"/]*)\.[^"]*)"(.*?)><a(.*?) href="([^"]*/)?(([^"/]*)\.[^"]*)"(.*?)>(.*?)</a></li>#','<a href="$6$7" target="_blank" >
        <div class="jrotty-links">
            <p>$10</p>
        </div>
    </a>',$str);
$str = preg_replace('#<li>([1-9][0-9]{4,})<a(.*?) href="([^"]*/)?(([^"/]*)\.[^"]*)"(.*?)>(.*?)</a></li>#','<a href="$3$4" target="_blank"><div class="jrotty-links"><p>$7</p></div></a>',$str);
}else{
...
$str = preg_replace('#<li>([1-9][0-9]{4,})<a(.*?) href="([^"]*/)?(([^"/]*)\.[^"]*)"(.*?)>(.*?)</a></li>#','<a href="$3$4" target="_blank">
	 ...
	 src="'.$loadtu.'"
	data-src="//q.qlogo.cn/g?b=qq&nk=$1&s=100">
            <p>$7</p>
        </div>
    </a>',$str);
}
